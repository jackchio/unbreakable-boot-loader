...
/*-----------------------------------------BOOT脚定义-----------------------------------------*/
#define BOOT_PIN        P35 //BOOT脚设置为P3.5脚,。
#define BOOT_RUN_APP    (0) //BOOT脚为低电平的时候运行APP。
#define BOOT_PIN_UPPULL do{P3PU=0x20;}while(0)//打开BOOT脚的上拉电阻。
#define BOOT_PIN_HIGHZ  do{P3PU=0x00;}while(0)//关闭BOOT脚的上拉电阻。
/*-----------------------------------------APP区定义-----------------------------------------*/
...
void uart_init(void){
    ...
    #endif  
    P1M0 &= ~0x03; P1M1 &= ~0x03; 
    UART2_SET_MODE_A_8_BAUD;//设置串口为8位无校验，1个停止位。
    UART2_SET_RXD_ENABLE;   //启动串口接收。
    TIMER2_SET_REG_HL((u16)(65536-(u16)(ECBM_SYSCLK_SETTING/4/ECBM_UART_BAUD_RATE)));//设置波特率。
    TIMER2_SET_MODE_1T;     //设定定时器2为1T模式，
    TIMER2_SET_MODE_TIMER;  //设定定时器2为16位自动重装方式，目前这样设置已经能满足常用的波特率了。
...
void uart_char(u8 ch){
    ...
    #endif
    UART2_SET_REG_S2BUF(ch);     //发送数据。
    while(!UART2_GET_TI_FLAG);  //等待发送完成。
    UART2_CLR_TI_FLAG;          //清除发送完成标志。
    #if   (UART_485_EN)
...
void main(void){        //main函数，必须的。
    ...
    while(1){           //本BootLoader不需要中断，全部在主循环里用查询法。
        if(UART2_GET_RI_FLAG){//串口1接收中断。
            UART2_CLR_RI_FLAG;//清掉串口1接收标志。
            switch(flash_state){
                //前三个状态用于判断帧头的AIC。
                case 0:{if(UART2_GET_REG_S2BUF=='A'){flash_state=1;}}break;
                case 1:{if(UART2_GET_REG_S2BUF=='I'){flash_state=2;}else{flash_state=0;}}break;
                case 2:{if(UART2_GET_REG_S2BUF=='C'){flash_state=3;}else{flash_state=0;}}break;
                case 3:{
                    flash_cmd=UART2_GET_REG_S2BUF;//帧头过后是指令，将指令存起来。
                ...
                case 4:case 5:{//STC8的最大容量就是64K，所以高位地址相当于没用。
                    ...
                    data_addr_ex+=(u16)(UART2_GET_REG_S2BUF);
                    flash_state++;
                }break;
                case 6:case 7:{
                    data_addr<<=8;//组装低位地址
                    data_addr+=(u16)(UART2_GET_REG_S2BUF);
                    flash_state++;
                ...
                case 8:case 9:case 10:case 11:{
                    data_len<<=8;//组装数据长度。
                    data_len+=(u16)(UART2_GET_REG_S2BUF);
                    flash_state++;
                ...
                case 12:{//编程数据指令。
                    flash_buf[data_count]=UART2_GET_REG_S2BUF;//接收数据到缓存。
                    data_count++;//数据计数器加一。
